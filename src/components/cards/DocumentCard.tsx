...
export function DocumentCard({ document }: { document: TechnicalDocument }) {
  const isRequestOnly = document.fileUrl.startsWith("request:");
  const href = isRequestOnly ? `/contact?document=${encodeURIComponent(document.id)}` : document.fileUrl;
  const actionLabel = isRequestOnly ? "Request document" : "Download";
  ...
  return (
    <article className="rounded-sm border border-black/10 bg-white p-5 shadow-soft">
      ...
      <a href={href} className="mt-5 inline-flex items-center gap-2 rounded-sm bg-forest-700 px-4 py-2 text-sm font-semibold text-white hover:bg-forest-900">
